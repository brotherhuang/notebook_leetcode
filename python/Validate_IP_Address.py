...
class Solution(object):
    def validIPAddress(self, IP):
        ...
        :rtype: str
        """
        def validIPv6(IPblock):
            if len(IPblock) < 1 or len(IPblock) > 4 : return False
            validChars = '0123456789abcdefABCDEF'
            for c in IPblock:
                if validChars.count(c) == 0 : return False
            return True
        def validIPv4(IPblock):
            if len(IPblock) < 1 or len(IPblock) > 3 : return False
            if not IPblock.isnumeric(): return False
            if int(IPblock) > 255 : return False
            if IPblock[0] == '0' and len(IPblock) > 1: return False
            return True
        IPv4 = IP.split('.')
        if len(IPv4) == 4:
            for block in IPv4:
                if not validIPv4(block) : return "Neither"
            return "IPv4"
        IPv6 = IP.split(':')
        if len(IPv6) == 8 :
            for block in IPv6:
                if not validIPv6(block): return "Neither"
            return "IPv6"
        return "Neither"    
